...
class Nodo():
    ...
    def altura(self)-> int:
        # si   no  tengo  hijos  la altura es -1
        # el nodo  se  encarga de  calcular  la altura
        alt_izq= -1
        if self._izq != None:
           alt_der = self._izq.get_altura()

        alt_der = -1
        if self._der != None:
            alt_der = self._der.get_altura()
...
class ArbolBalanceado():
    ...
    def rotar_derecha(self,nodo:Nodo, nodo_padre:Nodo):    
        nodo_central = nodo.get_izq()
        nodo_padre.set_izq(nodo.get_izq()) # podria estar a la derecha
        nodo_central.set_der(nodo)
        nodo.set_izq(None)

    def rotar_izquierda(self,nodo:Nodo, nodo_padre:Nodo):
        nodo_central = nodo.get_der()
        nodo_padre.set_der(nodo.get_der())
        nodo_central.set_izq(nodo)
        nodo.set_der(None)

    def rotar_doble_derecha(self,nodo:Nodo, nodo_padre:Nodo):
        pass

    def rotacion_simple(self,direccion):
        nodo_arriba=self._raiz
        if direccion == True:
            # se  vaya parala  derecha
            # hay 3 nodos 
            # el nodo de enmedio es la izquierda del nodo actual
            nodo_enmedio = nodo_arriba.get_izq()

            # los nodos tienen _enmedio
            
            # la derecha del de enmedio va a ser el nodo_arriba
            nodo_enmedio.set_der(nodo_arriba)
            

            # actuali
            #AHORA  el nodo de la izquierda tambien tiene qye subir

        else:
            nodo_enmedio = nodo_arriba.get_der()
            nodo_enmedio.set_izq(nodo_arriba)

    def rotacion_doble(self, direccion):
        # dos rotaciones simples
        nodo_actual=self._raiz
        if direccion==True:
            nodo_actual.set_izq(self.rotacion_simple(nodo_actual.get_izq(),False))
            nodo_actual= self.rotacion_simple(nodo_actual,True)
        else:
            nodo_actual.set_der(self.rotacion_simple(nodo_actual.get_der(),True))
            nodo_actual= self.rotacion_simple(nodo_actual,False)
