# IMPLEMENTACION DE UN ARBOL BINARIO BALANCEADO
...
class MyQueue:
    def __init__(self) -> None:
        # SON LOS FUTUROS INDICES DE LA COLA
        self.data: list[Any] = []
        self.head: int = 0
        self.tail: int = 0
...
class MyNode:
    def __init__(self, data: Any) -> None:
        self.data = data
        # FUTURES ## , ES DECIR QUE VA A SER UNA INSTANCIA DE LA MISMA CLASE
...
def right_rotation(node: MyNode) -> MyNode:
    r"""
       ...
       /
     UB
    UB = nodo no balanceado
    """
    print(Fore.YELLOW+"Rotar el nodo derecho:", node.get_data(), Fore.WHITE)
    ret = node.get_left()
...
def left_rotation(node: MyNode) -> MyNode:
    ...
    print(Fore.YELLOW + "Rotar nodo izquierdo:", node.get_data(), Fore.WHITE)
    ret = node.get_right()
    assert ret is not None
