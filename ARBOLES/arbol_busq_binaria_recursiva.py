...
class BinarySearchTree:
    ...
    def _put(self, node: Node | None, label: int, parent: Node | None = None) -> Node:
        # si el nodo es None, entonces creamos un nuevo nodo
        if node is None:
            node = Node(label, parent)
        # si el nodo no es None, entonces debemos buscar en que lado del nodo estamos
        # y  seguir buscando  hasta encontrar un nodo vacio
        else:
            if label < node.label:
...
def binary_search_tree_example() -> None:
    ...
    # vamos a nver que valores pertenecen al arbol
    ...
    # Valores del arboq  de menor a mayor
    inorder_traversal_nodes = [i.label for i in t.inorder_traversal()]
    print("Inorder traversal:", inorder_traversal_nodes)

    # Valoores del arbol de mayor a menor
    preorder_traversal_nodes = [i.label for i in t.preorder_traversal()]
