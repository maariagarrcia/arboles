...
# el camino más largo desde la raíz hasta una hoja no es más largo que dos veces el 
# camino más corto desde la raíz a una hoja
# ningún camino puede tener dos nodos rojos seguidos si lo estan vamos a cambiar uno
# El camino más corto posible tiene todos sus nodos negros, y el más largo alterna entre nodos rojos y negros
...
class RedBlackTree:
    ...
    def rotate_left(self) -> RedBlackTree:
        # girar a la izquierda el nodo
        # si no tiene hijo izquierdo no se puede girar
        # si tiene hijo izquierdo se gira 
        parent = self.parent
        right = self.right
        if right is None:
            return self
    ...
    def rotate_right(self) -> RedBlackTree:
        # girar a la derecha el nodo
        # si no tiene hijo derecho no se puede girar
        # si tiene hijo derecho se gira
    ...
    def _insert_repair(self) -> None:
        ## funcion para  cambiar el color despues de balancear un trozo del arbol
        if self.parent is None:
            # Necesitamos que la raíz sea negra
            self.color = 0
        elif color(self.parent) == 0:
            # Necesitamos que la  raiz  sea roja
            self.color = 1
        else:
